...
# Matches data:<mime>;base64,<payload> -- used only to shorten the
# payload for the *readable* view-source display. The actual download
# always serves the untouched full file (truncating there would break it).
_DATA_URI_RE = re.compile(r"(data:[^;]+;base64,)([A-Za-z0-9+/=]+)")
_READABLE_TRUNCATE_AT = 120  # chars of base64 payload to keep visible


def _make_readable(html: str) -> str:
    """
    Shortens long base64 data-URI payloads (images, fonts) so the HTML is
    actually readable/copyable as source code, instead of being dominated
    by single unbroken lines that can be hundreds of KB long. Only used
    for the /api/view-source display -- /api/download always returns the
    real, untruncated file so it still works when opened as a page.
    """
    def _shorten(match: re.Match) -> str:
        prefix, payload = match.group(1), match.group(2)
        if len(payload) <= _READABLE_TRUNCATE_AT:
            return match.group(0)
        size_kb = len(payload) * 3 / 4 / 1024  # approx decoded size
        return f"{prefix}{payload[:_READABLE_TRUNCATE_AT]}... [truncated for readability, ~{size_kb:.0f}KB total]"

    return _DATA_URI_RE.sub(_shorten, html)
...
async def view_source(job_id: str):
    """
    Returns a READABLE version of the generated HTML as plain text --
    open this URL directly to see the raw single-file HTML/CSS/JS code,
    with long base64 image/font payloads shortened so the source is
    actually scannable/copyable instead of dominated by giant unbroken
    lines. Use /api/download for the real, complete file.
    """
    job = JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found or already expired.")
    return HTMLResponse(content=_make_readable(job["html"]), media_type="text/plain")
...
async def extract_metadata_endpoint(req: MetadataRequest):
    ...
    When include_related is True (default), also looks for outgoing
    product-page links (e.g. a "recommended for you" rail) and returns
    lightweight metadata for each under `related_products`.
    """
    url_str = str(req.url)
    ...
    async with JOB_SEMAPHORE:
        try:
            metadata = await asyncio.wait_for(
                scrape_and_extract_metadata(url_str, device_type=req.device, include_related=req.include_related),
                timeout=settings.JOB_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
